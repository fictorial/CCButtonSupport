...
+ (id) buttonWithSprite:(CCSprite *) sprite label:(CCLabelTTF*) label target:(id) target selector:(SEL) selector
{
    return [[[self alloc] initWithSprite:sprite label:label target:target selector:selector] autorelease];
}

- (id) initWithImageFile:(NSString*) filename label:(CCLabelTTF*) labelIn target:(id) targetIn selector:(SEL) selectorIn
{
	CCSprite* sprite = [CCSprite spriteWithFile:filename];
    return [self initWithSprite:sprite label:labelIn target:targetIn selector:selectorIn];
}

- (id) initWithSprite:(CCSprite *) sprite label:(CCLabelTTF*) labelIn target:(id) targetIn selector:(SEL) selectorIn
{
    label = labelIn;
